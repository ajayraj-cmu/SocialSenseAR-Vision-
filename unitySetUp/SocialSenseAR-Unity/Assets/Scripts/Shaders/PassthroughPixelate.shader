    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                // Determine pixelation alpha based on mode:
                float maskAlpha;
                bool isInverted = _OutOfBoundsAlpha > 0.5;

                if (isInverted)
                {
                    // INVERTED PIXELATE (pixelate everything except target):
                    if (outOfBounds)
                ...
                {
                    // NORMAL PIXELATE (pixelate only the target):
                    // Hard cutoff — any partial alpha composites pixelated over sharp
                ...
                // Snap UV to block grid for pixelation effect
                float blockSize = max(1.0, _PixelBlockSize);
                float2 blockSizeUV = blockSize / _TextureResolution;
                float2 snappedUV = (floor(clampedUV / blockSizeUV) + 0.5) * blockSizeUV;
                snappedUV = clamp(snappedUV, 0.001, 0.999);

                // Sample the raw passthrough at the snapped (block-center) UV
                half4 pixelated = SAMPLE_TEXTURE2D(_PassthroughTex, sampler_PassthroughTex, snappedUV);

                pixelated.a = maskAlpha * _GlobalAlpha;
